...
// Cold-boot resilience tuning for the discovery socket.
const BIND_RETRY_MS      = 5 * 1000;
...
export function DiscoveryService()
{
    ...
    this.startSocketServer = function()
    {
        ...
        // At cold boot the adapter is often not ready yet and this bind fails.
        // The stock plugin bound once and never retried, which is the main
        // reason Govee devices come up dark after a reboot: every reply from
        // the device lands on 4002 and there was nothing listening.
        const now = Date.now();
        if (this.lastBindAttempt && (now - this.lastBindAttempt) < BIND_RETRY_MS) return;
        ...
        try
        {
            ...
            this.socketStartedAt = now;
        } catch(ex)
        {
            service.log('Could not bind UDP port 4002, will retry: ' + ex.message);
            this.udpServer = null;
        }
    }

    // If 4002 is bound but nothing has ever arrived, the bind almost certainly
    // did not really take (adapter down at boot, firewall still deciding).
    // Tear it down and try again instead of sitting there deaf.
    this.checkSocketHealth = function()
    {
        if (!this.udpServer) { this.startSocketServer(); return; }
        if (Object.keys(this.GoveeDeviceControllers).length === 0) return;

        const now = Date.now();
        const reference = this.lastMessageAt ? this.lastMessageAt : this.socketStartedAt;
        if (!reference) return;

        if ((now - reference) > SOCKET_SILENCE_MS)
        {
            service.log('No Govee traffic on 4002 for ' + Math.round((now - reference) / 1000) + 's, rebinding');
            try { this.udpServer.close(); } catch(ex) { /* already gone */ }
            this.udpServer = null;
            this.lastBindAttempt = 0;
            this.socketStartedAt = null;
            this.lastMessageAt = null;
